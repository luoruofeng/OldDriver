...
(function($) {
						...
						if(s.get(0)
							...
							if(n.changeInput) {
								switch(typeof n.changeInput) {
									case "boolean":
										o = $('<div class="jFiler-input" style="border: 2px solid #1B9AF7;width:62%"><div class="jFiler-input-caption"><span>' + n.captions.feedback + '</span></div><div class="jFiler-input-button">' + n.captions.button + '</div></div>"');
										break;
									case "string":
									case "object":
										o = $(n.changeInput);
	...
	$.fn.filer.defaults = {
		...
		captions: {
			...
			feedback2: "图片被选中",
			drop: "拖动图片进行上传",
			removeConfirmation: "移除图片？",
			errors: {
				filesLimit: "只有 {{fi-limit}} 被允许上传",
				filesType: "只有图片被允许上传",
				filesSize: "{{fi-name}} 太大！请上传 {{fi-maxSize}} MB 以内的图片",
				filesSizeAll: "你选择的图片太大!请上传 {{fi-maxSize}} MB 以内的图片"
			}
		}
	}
})(jQuery);